...
/*
用途: Metrics 是用来统计算法操作次数的记录结构，用于性能分析／计数（benchmark）。
字段说明:
compares：记录比较次数（例如在查找或定位插入位置时，每次比较 id 都应自增一次）。
moves：记录数据移动次数（例如数组元素赋值或插入/删除时的移动操作，每次赋值可计一次）。
在代码中的用法: 文件里很多函数的签名包含 Metrics *m。如果调用者传入非 NULL 的 m，函数在关键点会写成 if (m) m->compares++; 或 if (m) m->moves++; 来累加统计；若传入 NULL 则不统计（方便在不需要测量时跳过开销）。
为何用 long long: 为了容纳较大的计数（例如 N=20000 或更大规模的多次操作），避免溢出。
典型场景: 用来比较无序数组与有序数组在插入、查找、删除等操作上的比较/移动开销，从而评估算法复杂度与实际性能。
使用示例:
初始化并启用统计：Metrics m = {0,0}; uaFindInterestById(arr, n, id, &m);
不统计：把 NULL 传进去：uaFindInterestById(arr, n, id, NULL);
*/
